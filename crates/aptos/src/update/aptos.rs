...
impl BinaryUpdater for AptosUpdateTool {
    ...
    fn get_update_info(&self) -> Result<UpdateRequiredInfo> {
        ...
        let target_version_tag = latest_release.version;
        let target_version = target_version_tag.split("-v").last().unwrap();

        // Return early if we're up to date already.
        let current_version = cargo_crate_version!();
        let update_required = bump_is_greater(current_version, target_version)
