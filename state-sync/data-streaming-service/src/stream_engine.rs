...
impl ContinuousTransactionStreamEngine {
    ...
    fn create_subscription_stream_requests(
        ...
        // Determine the maximum number of in-flight requests
        ...
        let max_in_flight_requests = if prefetching_config.enable_dynamic_prefetching {
            // Prefetching is enabled. Use the max number of in-flight subscriptions
            prefetching_config.max_in_flight_subscription_requests
        } else {
            // Otherwise, use the statically configured max
            self.data_streaming_config.max_concurrent_requests
        };
